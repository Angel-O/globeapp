...
object MainShell extends BulmaCssClasses with Connect {
  ...
  def render(content: HTMLElement) = {
    ...
    val logoutButton = <Button label="logout" onClick={logOut _}/>

    val navbar = <Navbar
                    isFixedTop={false} 
                    isTransparent={true}
                    logo={logo}
                    rightItems={Seq(<NavbarItem item={logoutButton}/>)}/>
