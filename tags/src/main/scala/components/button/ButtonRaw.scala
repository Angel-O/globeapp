...
case class ButtonBuilderRaw() extends ButtonBaseBuilder {
  ...
  @dom def build = ButtonRaw(label, onClick, isDisabled().bind, isPrimary).bind.asInstanceOf[HTMLElement]
}
...
case class ButtonRaw private(
  ...
  @dom private def className = s"button ${if (clicked.bind) "is-focused"} ${if(isPrimary) "is-primary"}"
...
case object ButtonRaw {
  ...
  def apply(label: String, onclick: () => Unit, disabled: Boolean = false, primary: Boolean = false): Binding[HTMLElement] = {
