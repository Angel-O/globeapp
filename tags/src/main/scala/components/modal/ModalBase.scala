  ...
  private val closeOnClickOutside = () => {
    ...
    def getModal: HTMLElement = {     
      ...
      val modalBody = modal match {
        case null => null // Note the modal might be null for some weird reasons so wee need to check
                          // against that...probably timing issue when the component is re-mounted
        case _ => modal.getElementsByClassName(modalContentClassName).head.asInstanceOf[HTMLElement]
      }    
      modalBody
    }
    document.addEventListener("click", (e: Event) => {
      ...
      if(modal != null && isOpen.value && !modal.contains(e.target.asInstanceOf[HTMLElement])){
         closeModal(e)
      }
    }, true) // need to set useCapture to true to get the actual element  
  }
}